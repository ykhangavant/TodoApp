...
const emailConfig = {
    ...
    from: {
        ...
        name: process.env.EMAIL_FROM_NAME || 'Todo',
    },
};
...
const config = {
    allowedOrigins: process.env.TODO_ALLOWED_ORIGINS
        ? process.env.TODO_ALLOWED_ORIGINS.split(',').map((origin) =>
              origin.trim()
          )
        : [
              'http://localhost:8080',
    ...
    password: process.env.TODO_USER_PASSWORD,
    ...
    uploadPath:
        process.env.TODO_UPLOAD_PATH || path.join(projectRootPath, 'uploads'),
